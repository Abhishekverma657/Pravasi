...
export default function MediaBlogModal({ open, form, setForm, onSave, onClose }) {
  ...
  useEffect(() => {
    if (form.image instanceof File) {
      ...
      setPreview(form.image.startsWith("http") ? form.image : `${BASE}${form.image}`);
    } else {
      setPreview("");
    }
  }, [form.image, open]);
  ...
  const handleImage = (e) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      // // fallback to native alert if Toaster isn't mounted / is hidden
//       if (typeof document !== "undefined" && !document.querySelector(".react-hot-toast")) {
//  toast.error("File size too large. Maximum allowed is 5MB");
//       }
      return;
    }
  ...
  const handleSave = () => {
    ...
    onSave({
      ...safeForm,
      id: safeForm._id || safeForm.id || null,
      image: safeForm.image,
    });
  };
          ...
          >
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {/* Image Section */}
                <div className="col-span-1 flex flex-col items-center">
                  <div className="w-40 h-40 border border-gray-300 rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center">
                    <img
                      src={preview || PLACEHOLDER}
                      alt="Preview"
                      className="object-cover w-full h-full"
                  ...
                  <p className="text-xs text-gray-500 mt-2 italic">
                    Recommended: Upload a 16:9 image (e.g., 1280×720 px)
                  </p>
                ...
                <div className="col-span-2 flex flex-col gap-4">
                  ...
                  <input
                    ...
                    value={safeForm.subtitle}
                    onChange={(e) =>
                      setForm({ ...safeForm, subtitle: e.target.value })
                    }
                    placeholder="Subtitle"
                    className="w-full p-3 rounded-lg bg-white/70 text-gray-800 placeholder-gray-500 focus:ring-2 focus:ring-[#EBA832]"
                  />
                  ...
                  <textarea
                    value={safeForm.about}
                    onChange={(e) =>
                      setForm({ ...safeForm, about: e.target.value })
                    }
                    placeholder="About Blog"
                    rows={4}
                    className="w-full p-3 rounded-lg bg-white/70 text-gray-800 placeholder-gray-500 focus:ring-2 focus:ring-[#EBA832]"
